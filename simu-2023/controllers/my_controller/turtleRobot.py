...
from turtleMotors import TurtleMotors
from turtleLidar import TurtleLidar
from turtleDistanceSensors import TurtleDistanceSensors
...
class TurtleRobot(Robot):
    ...
    def __init__(self, timestep=None, camera_sampling_period=1):
        ...
        # Engines
        self.__motors = TurtleMotors()
        self.__speed = self.__motors.get_max_speed()
        # Sensors
        self.__lidar = TurtleLidar()
        self.__distance_sensors = TurtleDistanceSensors()
    ...
    def track_object(self, object_name):
        ...
        self.__lidar.track_object(object_name)

    def __process_recognition(self):

        if self.__lidar.is_tracked_object_on_left():
            self.__turn_left(capacity=0.5)
        if self.__lidar.is_tracked_object_on_right():
            self.__turn_right(capacity=0.5)
        if self.__lidar.is_tracked_object_present():
    ...
    def __run(self):
        ...
        if round(self.getTime() % self.__lidar_sampling_period , 1) == 0.1:
